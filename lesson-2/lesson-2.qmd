...
Now we will construct and add geometry attributes for our sites.

```{r}

# function st_point() creates a single point from numeric vectors
# we need three point for each site record
p1 = st_point(c(5,2))
...
# function st_sfc() create a column
...
tbs_sites_sf = st_as_sf(sites, coords = c("longitude", "latitude"), crs = 4326)
...
st_write(tbs_sites_sf, "data/data.gpkg", layer = "tbs_sites", delete_layer = TRUE)
...
Vector attribute store non-spatial information about feature e.g. site type, size or height.
...
# read sites
sites = st_read("data/data.gpkg", layer = "tbs_sites")
...
sites_lba = sites 
  ...
  select(id, size_ha, period)
  

```

::: callout-tip
`%>%` vs `I>` - [The new R pipe \| R-bloggers](https://www.r-bloggers.com/2021/05/the-new-r-pipe/)
:::
...
grid_sf = st_sf(tbs_grid_sfc)

# change column name
st_geometry(grid_sf) <- "geom"

# add unique id to grid
grid_sf$grid_id = 1:nrow(tbs_grid_sf)
...
the command x\[y,\] subsets fetures of target x using features of source y.
...
write_sf(agri_area, "data/agri_area_ltm.shp")
